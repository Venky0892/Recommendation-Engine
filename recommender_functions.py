...
def popular_rec_filter(user_id, n_top, ranked_movies, years, genres):
    '''
    INPUT:
    user_id - the user_id (str) of the individual you are making recommendations for
    n_top - an integer of the number recommendations you want back
    ranked_movies - a pandas dataframe of the already ranked movies based on avg rating, count, and time
    years - list of years the user have referred
    genres - genres the user selected

    OUTPUT:
    top_movies - a list of the n_top recommended movies by movie title in order best to worst bases on genres and years
    '''
    # Selecting movies based on the years 
    ranked_movies = ranked_movies[ranked_movies['date'].isin([years])]
    # Aggregating the genres
    num_genre_match = ranked_movies[[genres]].sum(axis=1)
    # Selecting genres which are more than 0
    ranked_movies = ranked_movies.loc[num_genre_match > 0, :]
    # Ranking the movies with n_top
    top_movies = list(ranked_movies['movie'][:n_top])
