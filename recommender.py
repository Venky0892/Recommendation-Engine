...
class Recommender():
    ...
    def make_recommendations(self, _id, _id_type='movie', rec_num=5, years = None, genres = None):
        ...
        rec_ids, rec_names, message = None, None, None
        if _id_type == 'user':
            if _id in self.user_ids_series:
                ...
                message = "Because this user wasn't in our database, we are giving back the top movie recommendations for all users."

        # Popular rec filter to identify movies based on year and genres 
        elif years and genres:
            rec_names = rf.popular_rec_filter(user_id = None, n_top = None, ranked_movies = self.ranked_movies, years = years , genres = genres)
            message = "Top ranked movies in that particular year and genres"
